...
bool hw_interface_plugin_roboteq::roboteq_drive::implDataHandler(const long &length, int arrayStartPos)
{
    ...
    try{
        ...
        {
            std::string command = tok_iter->c_str();
            ++tok_iter;

            if (command == "CB") // Absolute Brushless Counter
            {
              ROS_INFO("Absolute Brushless Counter <%s>",command.c_str());
            ...
            if (command == "A") // Motor Amps
            {
              ROS_INFO("Motor Amps <%s>",command.c_str());

              if(tok_iter != tokens.end())
              {
                  ROS_INFO("%s",tok_iter->c_str());
                  encoderData.motor1_amps = boost::lexical_cast<float>(tok_iter->c_str());
                  ++tok_iter;
              }
              ...
              {
                  ROS_INFO("%s",tok_iter->c_str());
                  encoderData.motor2_amps = boost::lexical_cast<float>(tok_iter->c_str());
                  ++tok_iter;
              }
